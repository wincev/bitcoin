Solidar Core version 0.11.0 is now available from:

  <https://solidar.org/bin/solidar-core-0.11.0/>
...
For example, add the following to `solidar.conf`:
...
related to the blockchain in the solidar system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Solidar Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Solidar Core will 
...
For an example script see [solidar-submittx](https://github.com/laanwj/solidar-submittx).
...
- #5334 `cf87536` libsolidarconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new solidar-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only SolidarAmountField
- #5753 `0f44672` Add solidar logo to about screen
...
- #5809 `46bfbe7` Add solidar-cli man page
...
- #5849 `41259ca` contrib/init/solidard.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Solidar-Qt.app to Solidar-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for solidar-tx)
