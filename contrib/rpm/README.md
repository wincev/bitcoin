...
The RPM spec file provided here is for Solidar-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://solidar.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Solidar git repository](https://github.com/solidar/solidar) but are not part of
...
Currently only `Source10` is used, for the example `solidar.conf` file.
...
in the solidar git repository. At present only an SVG version of the Solidar
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the solidar
...
needed when 0.12.1 is released, a proper fix is already in the Solidar git
master branch.
...
idea to build Solidar Core against a static version of that library compiled
...
The `solidar.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `solidar-server` package. At this point, most RPM based
...
The `solidar-server` package should have SELinux support. How to properly do
...
Solidar-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/solidar/). He has been
packaging Solidar for Fedora at least since 2012.
...
4. I renamed the `solidar` package that contains the Qt GUI to `solidar-core` as
that appears to be how the general population refers to it, in contrast to
`solidar-xt` or `solidar-classic`. I wanted to make sure the general population
